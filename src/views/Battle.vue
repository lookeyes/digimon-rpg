...
const won=ref(false),rewards=ref({gold:0,expPer:0,fieldExp:{}}),levelUps=ref([]),playerMaxLv=ref(1),lastMapField=ref(null),lastLevelMin=ref(1)
...
async function finishBattle(){
  ...
  if(w){winStreak.value++;const r=engine.value.getRewards();rewards.value=r;try{const rs=await saveBattleResults(engine.value.playerTeam,engine.value.enemyTeam,r);levelUps.value=rs.filter(r=>r.levelUps.length>0).map(r=>`${r.name}→Lv.${r.level}`)}catch(e){console.error(e)}}
...
async function useItemOn(it,target){if(!it||!target.alive)return;targeting.value=false;const d=itemDefs[it.id];if(!d)return;playerItems.value[it.id]=Math.max(0,(playerItems.value[it.id]||0)-1);if(d.effect==='healHp'){target.heal(target.maxHp);engine.value.log(`${target.name} 回复了全部HP！`)}else if(d.effect==='healMp'){target.restoreMp(target.maxMp);engine.value.log(`${target.name} 回复了全部MP！`)}else if(d.effect==='cure'&&target.status===d.status){target.status=null;engine.value.log(`${target.name} 的异常状态被治愈了！`)}else if(d.effect==='cureAll'){target.status=null;engine.value.log(`${target.name} 的全部异常状态被治愈了！`)};try{const u=getCurrentUser();if(u)await api.updateUser(u.objectId,{items:JSON.stringify(playerItems.value)})}catch(e){};selectedItem.value=null;engine.value._nextTurn();engine.value.onStateChange()}
