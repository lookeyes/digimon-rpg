...
const sEquip=computed(()=>{const st={...s.value};if(!st.maxHp)return st;const eq=equipData.value
  if(eq.badge){const b=eq.badge;const p=b.value/100
    if(b.stat==='all'){st.atk=Math.round(st.atk*(1+p));st.def=Math.round(st.def*(1+p));st.spAtk=Math.round(st.spAtk*(1+p));st.spDef=Math.round(st.spDef*(1+p));st.spd=Math.round(st.spd*(1+p))}
    else if(b.stat==='hp'){st.maxHp=Math.round(st.maxHp*(1+p));st.hp=Math.round(st.hp*(1+p))}
    else if(b.stat==='mp'){st.maxMp=Math.round(st.maxMp*(1+p));st.mp=Math.round(st.mp*(1+p))}
    else if(st[b.stat]!==undefined){st[b.stat]=Math.round(st[b.stat]*(1+p))}}
  if(eq.digivice?.stats){for(const[k,v]of Object.entries(eq.digivice.stats)){const p=v/100
    if(k==='hp'){st.maxHp=Math.round(st.maxHp*(1+p));st.hp=Math.round(st.hp*(1+p))}
    else if(st[k]!==undefined){st[k]=Math.round(st[k]*(1+p))}}}
  return st})
const maxStatVal=computed(()=>Math.max(sEquip.value.maxHp||1,sEquip.value.atk||1,sEquip.value.def||1,sEquip.value.spAtk||1,sEquip.value.spDef||1,sEquip.value.spd||1))
...
const statBars=computed(()=>!digimon.value?[]:[{key:'hp',icon:statIcons.hp,label:'HP',color:statColors.hp,percent:Math.round((sEquip.value.hp||0)/(sEquip.value.maxHp||1)*100),display:`${sEquip.value.hp||0}/${sEquip.value.maxHp||0}`},{key:'mp',icon:statIcons.mp,label:'MP',color:statColors.mp,percent:Math.round((sEquip.value.mp||0)/(sEquip.value.maxMp||1)*100),display:`${sEquip.value.mp||0}/${sEquip.value.maxMp||0}`},{key:'atk',icon:statIcons.atk,label:'攻击',color:statColors.atk,percent:Math.round((sEquip.value.atk||0)/maxStatVal.value*100),display:sEquip.value.atk||0},{key:'def',icon:statIcons.def,label:'防御',color:statColors.def,percent:Math.round((sEquip.value.def||0)/maxStatVal.value*100),display:sEquip.value.def||0},{key:'spAtk',icon:statIcons.spAtk,label:'特攻',color:statColors.spAtk,percent:Math.round((sEquip.value.spAtk||0)/maxStatVal.value*100),display:sEquip.value.spAtk||0},{key:'spDef',icon:statIcons.spDef,label:'特防',color:statColors.spDef,percent:Math.round((sEquip.value.spDef||0)/maxStatVal.value*100),display:sEquip.value.spDef||0},{key:'spd',icon:statIcons.spd,label:'速度',color:statColors.spd,percent:Math.round((sEquip.value.spd||0)/maxStatVal.value*100),display:sEquip.value.spd||0}])
...
const equipData = computed(() => { const d = digimon.value; if (!d?.stats) return {}; try { const s = typeof d.stats === 'string' ? JSON.parse(d.stats) : d.stats; return s._equip||{} } catch(e) { return {} } })
